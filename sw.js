// 6.9.75: 🛟 بوتِ حافظه‌اول + تایم‌اوت شبکه + پری‌کش مقاوم + ماژول صوتی و مکان‌یاب نقشه
var CACHE_NAME = "shahrdari-arak-v96";
var SW_VERSION = "6.9.75";
var TILE_CACHE_NAME = "map-tiles-v1";
var WORKER_HOST = "ops-sync.shahpanahhasan09.workers.dev";
var TILE_HOSTS = ["tile.openstreetmap.org", "arcgisonline.com", "basemaps.cartocdn.com"];
  ...
  e.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      var jobs = APP_SHELL.map(function (name) {
        var resolvedUrl = new URL(name, self.registration.scope).toString();
        return fetchWithTimeout(new Request(resolvedUrl, { cache: "no-store" }), 15000).then(function (res) {
          if (res && (res.ok || res.type === "opaque")) return cache.put(new Request(resolvedUrl), res.clone()).then(function(){ return cache.put(name, res); });
